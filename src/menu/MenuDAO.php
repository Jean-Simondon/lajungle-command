...
class MenuDAO
{
	...
	{
        $this->config_menu = [];
        $this->config_menu = $this->buildConfigMenu( SCRIPT_DIR . "/scripts_vendor" );
    ...
    private function buildConfigMenu($dirPath)
    {
        ...
        if( is_dir( $dirPath ) ) {
            ...
            foreach($dirInfo as $fileName) {
                if('.gitkeep' == $fileName || "meta.php" == $fileName ||  "todo" == $fileName || '.' == $fileName || '..' == $fileName ) {
                    continue;
                }
                ...
                if ( is_file( $dirPath . DS . $fileName ) ) {
                    $config["scripts"][] = [
                        ...
                        "value" => str_replace( SCRIPT_DIR . DS , "", ( $dirPath . DS . $fileName ) ) // chemin pour lancer le script
                    ];
                }
