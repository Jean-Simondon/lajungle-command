...
if ($this->get("CONTINUE") == false) :
    ...
    $this->display("Installation Bedrock")
        ...
        ->shell_exec("cd " . LJD_PROJECT_ROOT . "/ && mv bedrock/* ./")
        ->shell_exec("cd " . LJD_PROJECT_ROOT . "/ && mv bedrock/.* ./")
        ->shell_exec("cd " . LJD_PROJECT_ROOT . "/ && rm -Rf bedrock");
...
-
    ...
    $this->display("Installation WP_CLI")
        ...
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && curl -O https://raw.githubusercontent.com/wp-cli/builds/gh-pages/phar/wp-cli.phar')

        ->display("Initialisation BDD & sélection du thème")
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && php wp-cli.phar core install --url="' . $this->get("WP_HOME") . '" --title="' . $this->get("CAMEL") . '" --admin_user="' . $this->get("WP_USER") . '" --admin_password="' . $this->get("WP_PASSWORD") . '" --admin_email="' . $this->get("WP_EMAIL") . '" --skip-email')
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && php wp-cli.phar theme activate ' . $this->get("THEME_NAME"))
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && php wp-cli.phar language core install fr_FR')
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && php wp-cli.phar site switch-language fr_FR')
        // ->shell_exec('cd ' . LJD_PROJECT_ROOT . ' && php wp-cli.phar reset-cookieyes --activate')
    ...
    ->display("Déplacement des fichiers de scripts et ménage")
        ...
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && mv tmp/*.sh ./')
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && mv tmp/*.txt ./')
        ->shell_exec('cd ' . LJD_PROJECT_ROOT . '/ && rm -Rf tmp')
